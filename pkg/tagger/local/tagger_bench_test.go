...
func BenchmarkTagReadOnlyLowCardinality(b *testing.B) {
	tagger := initTagger()
	defer tagger.Stop()

	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			tagger.TagReadOnly("test", collectors.LowCardinality)
		}
	})
	b.ReportAllocs()
}
...
func BenchmarkTagReadOnlyHighCardinality(b *testing.B) {
	tagger := initTagger()
	defer tagger.Stop()

	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			tagger.TagReadOnly("test", collectors.HighCardinality)
		}
	})
	b.ReportAllocs()
}
